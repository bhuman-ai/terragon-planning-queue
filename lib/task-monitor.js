...
- Complete if: All proposal steps finished, success indicators present`;
  ...
  async notifyUser(task, message) {
    console.log(`📱 Sending WhatsApp notification for task ${task.id}: ${message}`);
    
    try {
      // Use the internal notification API which supports WhatsApp
      ...
      const response = await axios.post(notificationUrl, {
        ...
        message: message,
        channels: ['whatsapp', 'webhook', 'email'], // WhatsApp as priority
        urgency: 'normal'
      }, {
        headers: {
          'Content-Type': 'application/json',
      ...
      console.log(`✅ WhatsApp notification sent successfully:`, response.data.results);
      
      // Also try the webhook if configured
      if (this.notificationWebhook) {
        try {
          await axios.post(this.notificationWebhook, {
            ...
            whatsapp_sent: response.data.results.some(r => r.channel === 'whatsapp' && r.status === 'sent')
          });
        } catch (webhookError) {
