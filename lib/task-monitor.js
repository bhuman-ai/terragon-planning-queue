...
- Complete if: All proposal steps finished, success indicators present`;
  ...
  async continueTask(task, nextAction) {
    try {
      ...
      const githubRepoFullName = task.terragon?.githubRepoFullName || task.githubRepoFullName;

      if (githubRepoFullName) {
        try {
          const [owner, repo] = githubRepoFullName.split('/');
          const branch = task.terragon?.repoBaseBranchName || 'main';
          ...
          const claudeResponse = await axios.get(claudeUrl, { headers });

          if (claudeResponse.status === 200) {
            const claudeMdContent = claudeResponse.data;
          ...
          try {
            const taskResponse = await axios.get(taskUrl, { headers });

            if (taskResponse.status === 200) {
              const taskMdContent = taskResponse.data;
      ...
      if (contextParts.length > 0) {
        ...
        console.log(`Enriched task continuation with ${contextParts.length} context documents`);
      } else {
        enrichedMessage = nextAction;
      }

      // Send next action to Terragon
      await axios.post(`${this.terragonEndpoint}/${task.terragonSessionId}/messages`, {
   ...
   */
  async checkTaskNeedsInput(_task) {
    // This would check Terragon status or analyze current state
    // For now, return null (no input needed)
    return null;
  }
  ...
  async checkTaskCompletion(task) {
    ...
    if (task.progress && task.progress.completed === task.progress.total) {
      return true;
    }
    return false;
  }
  ...
  /**
   * Get task progress from Terragon
   */
  async getTaskProgress(task) {
    // This would check actual progress from Terragon
  ...
  async notifyCompletion(task) {
    await this.notifyUser(task, `✅ Task completed: ${task.title}`);
  }

  /**
   * Process Discord response and continue task
   */
  async processDiscordResponse(taskId, userResponse) {
    console.log(`📥 Processing Discord response for task ${taskId}: ${userResponse}`);
    ...
    try {
      ...
      // Send response to Terragon
      const baseUrl = process.env.VERCEL_URL || process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000';
      await axios.post(`${baseUrl}/api/actions/terragon`, {
        sessionToken: task.terragon?.sessionToken,
        message: userResponse,
        githubRepoFullName: task.terragon?.githubRepoFullName || 'user/project',
        repoBaseBranchName: 'main',
        enrichContext: true,
        isResponse: true,
        parentTaskId: task.terragon?.taskId
      });

      // Update task state
      task.status = 'executing';
      task.lastResponseAt = new Date().toISOString();
      task.responses = task.responses || [];
      task.responses.push({
        question: task.waitingFor?.question,
        answer: userResponse,
        timestamp: new Date().toISOString()
      });
      delete task.waitingFor;
      delete task.waitingForDiscordResponse;

      // Save updated task
      await this.saveTask(task);

      console.log('✅ Response sent to Terragon successfully');
      return { success: true, task };

    } catch (error) {
      console.error('❌ Failed to process Discord response:', error);
      throw error;
    }
  }

  /**
   * Get task by ID from KV or filesystem
   */
