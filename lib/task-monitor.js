...
- Complete if: All proposal steps finished, success indicators present`;
  ...
  async notifyUser(task, message) {
    console.log(`📱 Sending Discord notification for task ${task.id}: ${message}`);
    
    try {
      // Use the Discord notification API
      const baseUrl = process.env.VERCEL_URL || 'http://localhost:3000';
      const notificationUrl = `${baseUrl}/api/notifications/discord`;
